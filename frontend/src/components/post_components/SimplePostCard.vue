<template>
  <article
    class="max-w-3xl w-full flex flex-row items-start justify-between border border-gray-200 rounded-lg shadow overflow-hidden h-[125px]">
    <div class="w-full h-full p-4 flex flex-col justify-between">
      <div class="group relative flex flex-col h-full">
        <router-link :to="{ name: 'article', params: { id: post.id, slug: post.slug } }">
          ...
          <p class="mt-1 text-sm md:text-base leading-1 text-gray-600 !line-clamp-1" >{{PostPreview}}</p>
        </router-link>
      </div>
      <div>
        <div v-if="post.popular_tag">
        <Button ring :to="{ name: 'tag-detail', params: { slug: post.popular_tag.slug } }" size="sm"
          intent="secondary">{{ post.popular_tag.name }}</Button>
      </div>
    ...
    <div v-if="post.thumbnail" class="w-[50%] h-full">
      <router-link :to="{ name: 'article', params: { id: post.id, slug: post.slug } }" class="w-full h-full object-cover">
